...
"""
    ...
    Uses julia's built-in eigvals and eigvecs methods to return the eigenvalues and eigenvectors of a matrix A.
"""

function standardeigen(A)
	ev = eigvals(A)
	eek = eigvecs(A)
	return (ev,eek)
end
...
function goodness(standeig,testeig,m)
	...
	for i in 1:m  # iterates through all eigen pairs
		...
		if norm(vals[1,i]) > .99  # Change this number to whatever percent similarity/accuracy you desire
			count += 1
		end
	end
	return count
